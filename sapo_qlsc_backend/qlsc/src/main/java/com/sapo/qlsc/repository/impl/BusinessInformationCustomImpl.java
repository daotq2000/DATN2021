...
//    }
    ...
    public List<StatisticRepairmanDTO> getTopRepairMan(String startDate, String endDate) {
        String sql = "SELECT users.full_name as tenNV, count(c.code) as sophieuHT FROM sapo_qlsc.maintenance_cards c \n" +
                "right join users on c.repairman_id = users.id \n" +
                "where c.work_status = 2\n" +
                "and c.created_date BETWEEN Date(:startDate) AND Date(:endDate)\n" +
