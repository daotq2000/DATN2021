    ...
    const columns = [
        ...
        },
        {
            ...
            render: (warranty, data) => {
                return (<Checkbox checked={warranty === 1 ? true : false} onChange={() => warrantyItem(data.id)}></Checkbox>)
            }
        },
        {
            title: 'Số lượng',
        ...
        },
        {
            ...
            render: (status, data) => {
                if (data.type === 2) {
                    if (status === 0) {
                        return (
                            <Tag color="warning" onClick={()=>handleChangeStatus(data.maintenanceCardDetailId,status)}>Đang chờ</Tag>
                        )
                    }
                    else if (status === 1) {
                        return (
                            <Tag color="processing" onClick={()=>handleChangeStatus(data.maintenanceCardDetailId,status)}>Đang sửa</Tag>
                        )
                    }
                    else if (status === 2) {
                        return (
        ...
        <>
            ...
            <Row>
                <Table locale={{ emptyText: <Empty description='Không có sản phẩm' image={Empty.PRESENTED_IMAGE_SIMPLE}></Empty> }}
                    dataSource={products}
                    columns={columns} style={{ width: '100%' }} pagination={false} />
            </Row>
            <Row style={{ textAlign: 'right', marginTop: 30 }}>
                <h4 style={{ width: '100%' }}>Tổng tiền: {totalMoney()} đ</h4>
            </Row>
        </>
    );
}
