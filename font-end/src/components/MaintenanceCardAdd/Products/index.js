...
const Service = (props) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            width: '10%',
            render: (amount, data) => {
                if (data.type === 1 && user.role === 1 && props.maintenanceCardAdd.returnDate === null) {
        ...
        },
        {
            ...
            render: (warranty, data) => {
                if (warranty === 1) {
                    return (
                        user.role === 1 && props.maintenanceCardAdd.returnDate === null ? <Button onClick={() => warrantyItem(data.id)} type="primary">Bảo hành</Button> : <span>Bảo hành</span>
                    )
                }
                else {
                    return (
                        user.role === 1 && props.maintenanceCardAdd.returnDate === null? <Button onClick={() => warrantyItem(data.id)} >Không bảo hành</Button> : <span>Không bảo hành</span>
                    )
                }
            }
        },
        ...
        },
        {
            ...
            key: 'close',
            render: (key, data) => {
                if (user.role === 1 &&  props.maintenanceCardAdd.returnDate === null) {
                    if (data.status === 0) {
                        return (
    ...
    return (
        <>
            <Row>
                {user.role !== 2 && (props.maintenanceCardAdd.returnDate === null || props.maintenanceCardAdd.id === 0)  ? <AutoComplete
